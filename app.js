...
kingdoms
  ...
  // Watch own kingdom changes (sabotage/war/diplomacy sync)
  watchKingdom(user.uid, (kingdomData) => {
    if (!kingdomData) return;

    if (kingdomData.stats) {
      state.stats = {
        economy: kingdomData.stats.economy || 0,
        trust: kingdomData.stats.trust || 0,
        war: kingdomData.stats.war || 0,
        diplomacy: kingdomData.stats.diplomacy || 0
      };
    }
    ...
    state.multiplayer.ownedRegions = kingdomData.ownedRegions || ["Starting Province"];

    if (kingdomData.diplomacy) {
      state.multiplayer.allies = kingdomData.diplomacy.allies || [];
      state.multiplayer.enemies = kingdomData.diplomacy.enemies || [];
    }
...
function advanceTurn() {
  ...
  // Class Passive: Royal Kingdom restores +3 Trust every 5 turns
  if (state.player.focus === "monarch" && state.turn % 5 === 0) {
    state.stats.trust = Math.min(100, state.stats.trust + 3);
  }
...
renderAchievements();
  ...
  // Render the visual territory map
  renderTerritoryMap();
}
...
function renderTerritoryMap() {
  const container = document.getElementById("territory-map-grid");
  if (!container) return;

  const regions = state.multiplayer?.ownedRegions || ["Starting Province"];
  const territories = regions.length;

  container.innerHTML = "";

  // Territory count badge
  const badge = document.createElement("div");
  badge.className = "territory-count-badge";
  badge.innerHTML = `🗺️ <span>${territories} ${territories === 1 ? 'Territory' : 'Territories'} Controlled</span>`;
  badge.style.gridColumn = "1 / -1";
  container.appendChild(badge);

  regions.forEach((region, index) => {
    const tile = document.createElement("div");
    tile.className = `territory-tile${index === 0 ? " capital" : ""}`;

    const icon = REGION_ICONS[region] || "🏴";

    tile.innerHTML = `
      <span class="territory-icon">${icon}</span>
      <span class="territory-name">${region}</span>
      <span class="territory-badge">${index === 0 ? "👑 Capital" : `Region #${index + 1}`}</span>
    `;
...
if (createBtn) {
  ...
  // Hook up "Create World Vote" button
  const createVoteBtn = document.getElementById("btn-create-vote");
  if (createVoteBtn) {
    createVoteBtn.addEventListener("click", async () => {
      const title = prompt("Enter vote title:");
      if (!title || !title.trim()) return;

      const description = prompt("Enter vote description:");
      if (!description || !description.trim()) return;

      const success = await createPoliticsVote(
        title.trim(),
        description.trim(),
      ...
      if (success) {
        showToast("🗳️ World Vote created successfully!");
        audioSynth.playTriumph();
      } else {
        showToast("❌ Failed to create world vote.");
        audioSynth.playStrike();
      }
    });
  }
...
const headerInstallBtn = document.getElementById('btn-install-header');
...
window.addEventListener('beforeinstallprompt', (event) => {
  ...
  // Show custom gold install card and header button
...
window.addEventListener('appinstalled', () => {
  ...
  deferredInstallPrompt = null;
  console.log('Realmora installed successfully!');
});
...
function initProfileModalEvents() {
  ...
  if (modal.btnWar) {
    modal.btnWar.addEventListener('click', async () => {
      ...
      const result = await resolveWar(window.playerId, targetId);

      if (result) {
        // Set won_first_war flag if this player won
        if (result.winner === state.player.rulerName && !state.flags.includes("won_first_war")) {
          state.flags.push("won_first_war");
          saveGame();
        }
...
${result.attacker} Power: ${result.attackerScore}
${result.defender} Power: ${result.defenderScore}

🗺️ Territory Captured!
${result.capturedRegion}
`,
          true
        );
      }
...
// Set spy_sabotage_success flag for achievement
if (!state.flags.includes("spy_sabotage_success")) {
  state.flags.push("spy_sabotage_success");
  saveGame();
}

// Send sabotage alert to the defender
await sendInteraction(
  "sabotage_alert",
  window.playerId,
  state.player.rulerName,
...
function handleInteractionsUpdate(interactions, direction) {
  ...
  if (direction === "incoming") {
    interactions.forEach(inter => {
      ...
      // Handle sabotage alert (defender was sabotaged)
      if (inter.type === "sabotage_alert" && inter.status === "pending") {
        respondToInteraction(inter.id, "acknowledged");

        createNotificationCard({
          ...inter,
          id: inter.id + "_sabotage_notify",
        ...
        if (!activeNotificationCards.has(inter.id + "_accepted")) {
          ...
          if (inter.type === "alliance") {
            if (!state.multiplayer.allies.includes(inter.receiverId)) {
              ...
              // Class Passive: Diplomatic Federation gets +15 instead of +10
              const allianceBonus = state.player.focus === "embassy" ? 15 : 10;
              state.stats.diplomacy = Math.min(100, state.stats.diplomacy + allianceBonus);
              saveGame();
              if (window.playerId) {
                updateKingdom(window.playerId, {
                  "diplomacy.allies": state.multiplayer.allies,
            ...
            // Class Passive: Trade Dynasty (merchant) gets +15 instead of +10
            const tradeBonus = state.player.focus === "merchant" ? 15 : 10;
            state.stats.economy = Math.min(100, state.stats.economy + tradeBonus);
            saveGame();
            if (window.playerId) {
              updateKingdom(window.playerId, { stats: state.stats });
            }
...
function createNotificationCard(inter) {
  ...
  } else if (inter.type === "sabotage_received") {
    title = "🔥 ALERT: Kingdom Sabotaged!";
    msg = `Agents from <strong>${inter.senderKingdom}</strong> have infiltrated our realm and damaged our infrastructure! A random stat has been reduced by 15 points.`;
    audioSynth.playStrike();
  }
...
function renderWorldVotes(
  ...
  votes.forEach(
    ...
    card.querySelector(".btn-vote-yes").addEventListener("click", async () => {
      const res = await voteOnIssue(vote.id, window.playerId, "yes");
      if (res && res.success) {
        showToast("🗳️ Vote 'YES' cast!");
      } else {
        showToast(res && res.error ? `❌ ${res.error}` : "❌ Vote failed.");
      }
    });

    card.querySelector(".btn-vote-no").addEventListener("click", async () => {
      const res = await voteOnIssue(vote.id, window.playerId, "no");
      if (res && res.success) {
        showToast("🗳️ Vote 'NO' cast!");
      } else {
        showToast(res && res.error ? `❌ ${res.error}` : "❌ Vote failed.");
      }
    });
...
function renderLeaderboard(kingdoms) {
  const globalContainer = document.getElementById("global-leaderboard-list");
  const realmContainer = document.getElementById("realm-leaderboard-list");

  const buildList = (container, list) => {
    if (!container) return;
    container.innerHTML = "";
    if (list.length === 0) {
      container.innerHTML = `<p class="text-muted" style="font-size:0.8rem; text-align:center; padding:1rem 0;">No rulers ranked yet.</p>`;
      return;
    }

    const sorted = [...list].sort((a, b) => {
      const scoreA = (a.territories || 1) * 30 + (a.stats?.war || 50) + (a.stats?.economy || 50);
      const scoreB = (b.territories || 1) * 30 + (b.stats?.war || 50) + (b.stats?.economy || 50);
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            <div style="font-weight:700; color:#fff; font-size:0.8rem;">
              ${crown} #${index + 1} ${player.rulerName}
            </div>
          ...
          <div style="text-align:right; font-size:0.68rem;">
            🗺️ ${player.territories || 1}
            <br>
            ⚔️ ${player.stats?.war || 50}
          </div>
        </div>
      `;
      container.appendChild(card);
  ...
  buildList(globalContainer, kingdoms);
  
  // Filter for active/online players
  const activeKingdoms = kingdoms.filter(k => k.online);
  buildList(realmContainer, activeKingdoms);
}
